...
# IMPORTACIONES INSTITUCIONALES (FASE 1: RIESGO EXTREMO Y VALIDACIÓN)
try:
    from diagnostics.evt_var import calculate_evt_var_es
    ...
    st.error("Faltan módulos institucionales en la carpeta diagnostics/.")

# 1. INYECCIÓN DEL GENERADOR GLOBAL DE NÚMEROS ALEATORIOS (RNG)
# Esto asegura reproducibilidad institucional real en todas las capas estocásticas
GLOBAL_RNG = np.random.default_rng(42)

# ==============================================================================
# LÓGICA DE PDF Y SANITIZACIÓN
# ==============================================================================
...
def create_pdf_report(data: dict) -> bytes:
    ...
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(0, 8, txt=" 2. ESTRUCTURA ESTOCASTICA Y EXTREME VALUE THEORY (EVT)", ln=True, fill=True)
    pdf.set_font("Arial", '', 10)
    sym = data['sym']
    ...
    pdf.cell(0, 6, txt=clean_text_for_pdf(f"   - Capital en Riesgo (VaR EVT {data['conf_level']}%): {sym}{data['capital_var_evt']:,.2f}"), ln=True)
    pdf.cell(0, 6, txt=clean_text_for_pdf(f"   - Severidad Proyectada (ES EVT): {sym}{data['capital_es_evt']:,.2f}"), ln=True)
    pdf.ln(5)
...
def apply_defensive_overlay(current_mu_apf, raw_mu_historical, current_sigma, current_regime):
    """Evita la explosión geométrica anclando la deriva matemática al régimen empírico."""
    blended_mu = (0.2 * current_mu_apf) + (0.8 * raw_mu_historical)
    
    if current_regime == 0:
        safe_mu = 0.0 # Transición: Deriva neutral probabilística
    elif current_regime == -1:
        safe_mu = -abs(blended_mu) if blended_mu != 0 else -0.10 # Estrés: Forzar deriva negativa
    else:
        safe_mu = np.clip(blended_mu, -0.40, 0.40) # Expansión: Hard Clipping al 40% anualizado
        
    safe_sigma = max(current_sigma, 0.15) # Piso de volatilidad institucional
...
def run_time_machine(df_full, days_ago, simulations, trading_days):
    ...
    # APLICAMOS EL FRENO INSTITUCIONAL PARA BACKTESTS SEGUROS
...
def render_dashboard():
    ...
    with st.sidebar:
        ...
        override_ia = st.toggle("☢️ Stress Test Manual")
        if override_ia:
            st.error("Motor Variacional en Bypass. Simulando colas pesadas manuales.")
    ...
    with st.spinner("Calibrando Riesgo Estructural..."):
        apf_states = apply_auxiliary_particle_filter(daily_returns.values, num_particles=1000)
        pesos_ewma = np.exp(np.linspace(-1, 0, min(20, len(apf_states))))
        current_mu_apf = np.dot(apf_states[-len(pesos_ewma):], pesos_ewma / pesos_ewma.sum()) * trading_days
        ...
        # SEPARACIÓN INSTITUCIONAL: Freno de Gobernanza sobre la Inferencia
        safe_mu, safe_sigma_ann = apply_defensive_overlay(current_mu_apf, raw_mu, current_sigma_ann, current_regime)
    ...
    with st.spinner("Generando Trayectorias y Evaluando Colas (EVT)..."):
        paths = run_montecarlo_advanced_stochastic(
            S0, safe_mu, raw_mu, safe_sigma_ann, long_term_sigma_ann, 
            days_to_project, simulations, ml_lambda_j, ml_mu_j, ml_sigma_j, trading_days
        )
        
        prob_pos, var_price_gauss, median_price, tp_price = calculate_risk_metrics_phase1(S0, paths, conf_level)
        
        # EXTREME VALUE THEORY: Re-calibración de la Cola Terminal
        terminal_prices = paths[-1, :]
        alpha_level = 1.0 - (conf_level / 100.0)
        var_price_evt, es_price_evt, tail_model_status = calculate_evt_var_es(
            terminal_prices, S0, alpha=alpha_level, threshold_percentile=10.0
        )
    ...
    capital_var_evt = (capital_inicial / S0) * var_price_evt
    capital_es_evt = (capital_inicial / S0) * es_price_evt
    ...
    c3.metric("Tail Risk Model", tail_model_status)
    c4.metric("Deriva Ajustada", f"{safe_mu*100:.1f}%")
    ...
    with tab1:
        ...
        col_sl.error(f"**VaR EVT ({conf_level}%):**\n\n**${var_price_evt:,.2f}**\n\n*ES (Severidad): ${es_price_evt:,.2f}*")
        ...
        fig.add_trace(go.Scatter(x=[0, days_to_project], y=[var_price_evt, var_price_evt], mode='lines', line=dict(color='red', dash='dot')))
    ...
    with tab4:
        ...
        if st.button(f"Ejecutar Máquina del Tiempo (-{days_ago}d)"):
            with st.spinner("Rebobinando mercado y calculando PnL histórico..."):
                ...
                # CÁLCULOS FINANCIEROS (AHORA BLINDADOS)
    ...
    # --- EXPORTACIÓN ---
    report_data = {
        "ticker": ticker, "S0": S0, "tp_price": tp_price, "var_price": var_price_evt,
        "capital_var_evt": capital_var_evt, "capital_es_evt": capital_es_evt,
        ...
        "capital_esperado": capital_esperado, "capital_var": capital_var_evt,
        "rend_esp": rend_esp, "vol_dyn": safe_sigma_ann * 100, 
